...
export function PaginationContent({
  ...
  return (
    <ul
      className={cn(
        "flex flex-row items-center gap-1 sm:gap-1.5 flex-wrap justify-center",
        className,
      )}
      {...props}
    />
  ...
  return (
    <a
      ...
      className={cn(
        ...
        size === "default" &&
          "h-8 sm:h-9 px-2.5 sm:px-4 py-1.5 sm:py-2 gap-1 sm:gap-1.5 text-xs font-medium",
        size === "sm" && "h-7 sm:h-8 px-2 sm:px-3 text-xs",
        size === "lg" && "h-9 sm:h-10 px-4 sm:px-5 text-sm",
        size === "icon" && "h-8 w-8 min-w-8 sm:h-9 sm:w-9 sm:min-w-9",
        isActive
...
export function PaginationEllipsis({
  ...
  return (
    <span
      aria-hidden
      className={cn(
        "flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center text-slate-400",
        className,
      )}
      {...props}
    >
