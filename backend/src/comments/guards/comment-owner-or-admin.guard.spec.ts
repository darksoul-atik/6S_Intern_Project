...
import { CommentOwnerOrAdminGuard } from './comment-owner-or-admin.guard.js';
import type { CommentsService } from '../comments.service.js';

describe('CommentOwnerOrAdminGuard', () => {
  let guard: CommentOwnerOrAdminGuard;
  ...
  beforeEach(() => {
    ...
    guard = new CommentOwnerOrAdminGuard(mockCommentsService as CommentsService);
  });
  ...
  it('should allow access immediately if user has role admin without checking comment', async () => {
    ...
    expect(mockCommentsService.findCommentByIdOrThrow).not.toHaveBeenCalled();
  });
